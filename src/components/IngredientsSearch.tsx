...
const IngredientsSearch = ({ onSelectRecipe, savedIds, onToggleSave }: IngredientsSearchProps) => {
  ...
  const matchingRecipes = ingredientsList.length > 0
    ? recipes.filter(recipe =>
        ingredientsList.some(ing =>
          recipe.ingredients.some(ri =>
            ri[lang].toLowerCase().includes(ing) || ri.uz.toLowerCase().includes(ing) || ri.en.toLowerCase().includes(ing)
          )
        )
      )
    : [];
  ...
  return (
    <div className="p-4 pb-24 md:pb-8 animate-fade-in">
      <h2 className="font-display font-bold text-xl text-foreground mb-1">{t('ingredientSearch')}</h2>
      <p className="text-sm text-muted-foreground mb-4">
        {lang === 'uz' ? "Uyingizda bor mahsulotlarni kiriting va qanday taom qilish mumkinligini bilib oling" : "Enter ingredients you have at home and discover what you can cook"}
      </p>
